...
class DeckFactoryTest {
    ...
    @Test
    void mainDeck_hasExactly125Cards() {
        assertEquals(125, mainDeck.size());
    }
    ...
    void mainDeck_hasExactly72ColorCards() {
        long count = mainDeck.stream().filter(c -> c.type() == CardType.COLOR).count();
        ...
        for (CardColor color : colors) {
            for (int value = 1; value <= 9; value++) {
                ...
                long count = mainDeck.stream()
                        .filter(c -> c.type() == CardType.COLOR && c.color() == color && c.value() == v)
                        .count();
    ...
    void mainDeck_colorCards_valuesAreInRange1To9() {
        mainDeck.stream()
                ...
                .forEach(c -> assertTrue(c.value() >= 1 && c.value() <= 9,
    ...
    void mainDeck_hasExactly9BlackCards() {
        long count = mainDeck.stream().filter(c -> c.type() == CardType.BLACK).count();
    ...
    void mainDeck_hasExactly20SpecialSingleCards() {
        long count = mainDeck.stream().filter(c -> c.type() == CardType.SPECIAL_SINGLE).count();
    ...
    void mainDeck_specialSingleCards_exactly5PerColor() {
        ...
        for (CardColor color : colors) {
            long count = mainDeck.stream()
                    .filter(c -> c.type() == CardType.SPECIAL_SINGLE && c.color() == color)
                    .count();
            assertEquals(5, count, "Expected 5 SPECIAL_SINGLE cards for color " + color);
        }
    }
    ...
    void mainDeck_specialSingleCards_onlyHaveSingleColorEffects() {
        ...
        mainDeck.stream()
                ...
                .forEach(c -> assertTrue(singleEffects.contains(c.effect()),
    ...
    @Test
    void mainDeck_hasExactly23SpecialFourCards() {
        long count = mainDeck.stream().filter(c -> c.type() == CardType.SPECIAL_FOUR).count();
    ...
    void mainDeck_specialFourCards_countPerEffect() {
        ...
        assertEquals(4, countEffect(mainDeck, SpecialEffect.COUNTERATTACK));
    ...
    void mainDeck_hasExactly1FuckYouCard() {
        long count = mainDeck.stream().filter(c -> c.type() == CardType.FUCK_YOU).count();
        assertEquals(1, count);
    }

    @Test
    void mainDeck_fuckYouCard_hasId124() {
        Card fuckYou = mainDeck.stream()
                .filter(c -> c.type() == CardType.FUCK_YOU)
                .findFirst()
                .orElseThrow();
        assertEquals(124, fuckYou.id());
    }
    ...
    void mainDeck_cardIdsAreInRange0To124() {
        mainDeck.forEach(c -> assertTrue(c.id() >= 0 && c.id() <= 124,
    ...
    void eventDeck_allCardIdsAreUnique() {
        Set<Integer> ids = eventDeck.stream().map(Card::id).collect(Collectors.toSet());
    ...
    void shuffle_producesDifferentOrderFromOriginal() {
        ...
        List<Integer> originalIds = deck.stream().map(Card::id).collect(Collectors.toList());

        DeckFactory.shuffle(deck, new Random(42));
        List<Integer> shuffledIds = deck.stream().map(Card::id).collect(Collectors.toList());

        assertNotEquals(originalIds, shuffledIds, "Shuffled deck should differ from original order");
    }

    @Test
    void shuffle_preservesAllCards() {
        List<Card> deck = DeckFactory.buildMainDeck();
        Set<Integer> before = deck.stream().map(Card::id).collect(Collectors.toSet());

        DeckFactory.shuffle(deck, new Random(99));
        Set<Integer> after = deck.stream().map(Card::id).collect(Collectors.toSet());
    ...
    void shuffle_withSameSeed_producesSameOrder() {
        ...
        List<Integer> ids1 = deck1.stream().map(Card::id).collect(Collectors.toList());
        List<Integer> ids2 = deck2.stream().map(Card::id).collect(Collectors.toList());
    ...
    private long countEffect(List<Card> deck, SpecialEffect effect) {
        return deck.stream().filter(c -> c.effect() == effect).count();
    }
}
